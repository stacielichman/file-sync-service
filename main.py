...
from routes import SYNC_FOLDER_PATH, CloudFiles, UploadFiles
...
def get_cloud_files() -> dict:
    ...
    if cloud_files.status_code != 200:
...
def check_if_delete_file() -> None:
    ...
    with LOCK:
        ...
        if len(files_to_delete) == 0:
            return
        CloudFiles().delete(files_to_delete)
...
def check_if_upload_file() -> None:
    ...
    with LOCK:
        ...
        if len(files_to_upload) == 0:
            return
        UploadFiles().get(files_to_upload)
...
def check_if_reload_file() -> None:
    ...
    with LOCK:
        ...
        UploadFiles().get(files_to_patch)
